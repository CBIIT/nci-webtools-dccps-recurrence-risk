...
export class FileService {
  ...
  readFile(file: File, type: "arrayBuffer" | "dataUrl" | "text" = "text"): Promise<ArrayBuffer | string | null> {
  ...
  parseIni(contents: string, trimWhitespace: boolean = true, globalSection: string = "__global"): IniConfig {
    let ini: any = {};
   ...
   * @returns
   */
  async parseSeerStatDictionary(dictionaryFile: File): Promise<{ headers: DataFrameHeader[]; config: IniConfig }> {
   ...
   * @returns
   */
  async parseCsvFile(csvFile: File, options?: CsvParseConfig): Promise<DataFrame> {
   ...
   * @returns
   */
  async parseSeerStatFiles(seerStatDictionaryFile: File, seerStatDataFile: File): Promise<DataFrame> {
    const { headers, config } = await this.parseSeerStatDictionary(seerStatDictionaryFile);
