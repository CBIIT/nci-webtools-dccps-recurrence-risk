...
export function parseCsv(text: string, config?: Partial<CsvParseConfig>) {
  ...
  const addFieldToRecord = (record: Record, header: string, index: number): Record => ({
    ...record,
  ...
  const appendRecord = () => {
    ...
    if (numLines >= skipLines) {
      if (headers) {
        ...
        const record: Record = (headers as string[]).reduce(addFieldToRecord, {});
 ...
 * @returns
 */
export function stringifyCsv(data: Record[] | Value[][], config?: Partial<CsvStringifyConfig>) {
  ...
  const escape = (value: Value) => (typeof value !== "number" ? `"${String(value).replace(/"/g, '""')}"` : value);
