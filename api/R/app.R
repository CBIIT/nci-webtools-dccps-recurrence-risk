             ...
             tabPanel("Group Data",
                      sidebarLayout(
                        sidebarPanel(
                          ...
                          numericInput("r", "Adjustment Factor r:", value=1, min = 0.05, max = 5, step = 0.05),
                     ...
                     The current version can handle Weibull and log-logistic distributions for the non-cured survival."),
                       ...
                       </li><li> <b>SEER*Stat Data File</b>: the cause-specific survival or relative survival data generated from SEER*Stat in .txt format. Note that, if the .txt file is not exported from SEER*Stat
                            ...
                            tabPanel("Help",
                                     ...
                                     HTML("<b>Input</b>
                                          <ul><li> <b>CSV Data File</b>: the case-listing unformatted CSV data file exported from SEER*Stat software with modification of variables of interest or user-generated individual data by other statistical software in the CSV format. Note that, 
                                                                         users may not directly use the original unformatted variables, but need to modify the case-listing SEER*Stat data before uploading because 
                                                                         1) it may not contain the event variable for death due to cancer and the dead events due to other causes should be recoded as censoring events; 
...
server <- function(input, output,session) {
  ...
  seerdata<-reactive({
    ...
    cnames.seer <- colnames(data.seerstat)
    ...
    if(length(which(data.seerstat[,surv.name]>1))>0){
      data.seerstat[,surv.name]<-data.seerstat[,surv.name]*0.01
      data.seerstat[,survse.name]<-data.seerstat[,survse.name]*0.01
    }
  ...
  risk.out<-reactive({  
    ...
    ## convert to proportions if the survival rates are percentages -- moved to seerdata reactive function
